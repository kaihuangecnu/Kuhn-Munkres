...
public:
    ...
    int solve(Matrix<Data> &m) {
        ...
        // Copy input matrix
        this->matrix = m;
	this->matrix_backup = Matrix<Data>(size,size);
        for ( size_t row = 0 ; row < size ; row++ ) {
            for ( size_t col = 0 ; col < size ; col++ ) {
		this->matrix_backup(row,col) = m(row,col);
	    }
	}
	...
	double infinity = std::numeric_limits<double>::infinity();
        for ( int row = 0 ; row < size ; row++ ) {
                ...
                for ( int col = 0 ; col < size ; col++  ) {
                        if ( matrix(row,col) == 0 ) {
                                rowcount++;
				if (matrix_backup(row,col)==infinity) {
					return 1;		
				}
			}
                }
